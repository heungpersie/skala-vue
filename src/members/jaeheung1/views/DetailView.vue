<script setup>
/* 상세 기상관측 화면. 원래는 vue-router의 :cityId 파라미터로 도시를 찾았지만,
   이 폴더는 독립된 라우트 없이 쓰이므로 props로 cityId를 전달받는다. */
...
const city = computed(() => cities.value.find((c) => c.id === props.cityId) ?? null)
