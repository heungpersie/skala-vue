...
/* ── [Store 실습 / 과제5] WeatherStoreHomeView.vue ──
   ...
   전환할 수 있게 만든 메인 날씨 대시보드. 도시 목록/실시간 API 로직은 과제4와 동일하게
   useWeatherCities 싱글턴 상태를 그대로 공유한다. */
import { ref, computed, watch, watchEffect, onMounted } from 'vue'
import { useRouter } from 'vue-router'
import BaseDashboardCard from '@/components/exercise/BaseDashboardCard.vue'
import SearchBar from '@/components/exercise/SearchBar.vue'
import WeatherCardStore from '@/components/exercise/WeatherCardStore.vue'
...
const showDetail = (city) => {
  router.push('/weather-store/' + city.id)
}
...
<template>
  <div class="card-panel">
    <p class="panel-title">
      🌤️ 과제 5: 날씨 대시보드(스토어적용) <span class="live-dot" title="Open-Meteo 실시간 연동"></span>
    </p>
    <div class="link-row">
      <RouterLink to="/weather-store/about" class="about-link">ℹ️ 이 실습 소개</RouterLink>
      <RouterLink to="/weather-store/no-such-page" class="about-link">
        🚧 키키 보러가기 ->
      </RouterLink>
      <button
